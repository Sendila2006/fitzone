...
// Check if the user is already logged in as admin or staff
if (isset($_SESSION['username']) && ( $_SESSION['role'] == 'staff')) {
    echo "<script>alert('You are already logged in. Redirecting to homepage...');</script>";
    header("Location: staff_dashboard.php"); // Redirect to index.php if already logged in
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Secure query to validate credentials
    $sql = "SELECT * FROM users WHERE username = ? AND role IN ('staff')";
    ...
    if ($result->num_rows > 0) {
        ...
        if (password_verify($password, $row['pwd'])) {
            ...
            echo "<script>alert('You have successfully logged in as a Staff!');</script>";

            // Redirect to index.php after a short delay
            echo "<script>setTimeout(function(){ window.location.href = 'staff_dashboard.php'; }, 1000);</script>";
            exit();
        } else {
...
<div class="login-container">
    ...
    <form method="POST" action="staff.php">
        ...
        <br>
        <br>
        <center><a href="../index.php">Back To Home</a></p></center>
